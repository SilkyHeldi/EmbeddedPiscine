...
void	adc_init()
{
	...
	// we need to write 8 bits only so we do a left adjust result to get ADCH (8 bits read)
	ADMUX |= (1 << ADLAR);
...
int	main()
{
	...
	while (1)
	{
		// we ant RV1 which is on ADC0 (ADC_POT) (0000)
		// POTENTIOMETER
		ADMUX &= ~(1 << MUX3);
		ADMUX &= ~(1 << MUX2);
		ADMUX &= ~(1 << MUX1);
		...
		// we want LDR which is on ADC1 (ADC_) (0001)
		// LUMINOSITY
		ADMUX &= ~(1 << MUX3);
		ADMUX &= ~(1 << MUX2);
		ADMUX &= ~(1 << MUX1);
		...
		// we want NTC which is on ADC2 (ADC_) (0010)
		// TEMPERATURE
		ADMUX &= ~(1 << MUX3);
		ADMUX &= ~(1 << MUX2);
		ADMUX |= (1 << MUX1);
